...
dexpr float Objetivo =
  CostFlag * (sum(i in Usinas : i != "Demanda") Pot[i] * CF[i]) +
  (CostFlag - 1) * (sum(i,j in Usinas, t in Timeslice) Covariancia[t][i][j] * Pot[i] * Pot[j]);
...
subject to {
  ...
  forall (t in Timeslice) {
    ...
    ConstantCost: 
      (sum (i in Usinas) (Pot[i] * CFpos[i] * Custo[i])) == 0; 
  }
...
float MeanGen = sum(i in Usinas : i != "Demanda") Pot[i] * CF[i]; // in MWyear
